...
f_dict = open('ed4/dictionary.txt')# from scrabble words list
scrabble_list = []
for line in f_dict:
...
def convert_txt_to_clean(fname):
    ...
    # print full_text
    fr.close()
    fw = open(fname[:-4]+'_clean.txt', 'w') # final text final is stored as filename_clean.txt
...
def get_file_name(a,file_obj):
    ...
    print("Final video lists : {}".format(",".join(map(str,video_clip_list))))
    video_clip = concatenate_videoclips(video_clip_list)
    # print("And the final video clip",video_clip)
    directory_file = 'ed4/static/files/video-finished/' + infile.split('/')[-1].split('.')[0]+'.mp4'
